...
Tracking::Tracking() {
  ...
  noise_ax = 5; // sigma_squared ax
  noise_ay = 5; // sigma_squared ay
}
...
void Tracking::ProcessMeasurement(const MeasurementPackage &measurement_pack) {
  ...
  this->kf_.Q_ = MatrixXd(4,4);
  this->kf_.Q_ << 0.25*pow(dt,4)*noise_ax, 0, 0.5*pow(dt,3)*noise_ax, 0,
  ...
  this->kf_.Update(measurement_pack.raw_measurements_);
