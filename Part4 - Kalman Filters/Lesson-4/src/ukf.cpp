...
void UKF::GenerateSigmaPoints(MatrixXd* Xsig_out) {
  ...
  double eq_const = sqrt(lambda + n_x);
  // set sigma points as columns of matrix Xsig
  Xsig.col(0) = x;

  for (int i = 0; i < n_x; i++)
  {
    Xsig.col(i+1) = x + (eq_const * A.col(i));
    Xsig.col(i+n_x+1) = x - (eq_const * A.col(i));
  }
...
void UKF::AugmentedSigmaPoints(MatrixXd* Xsig_out) {
  ...
  // Assuming long. acceleration & raw acceleration mean == 0
  x_aug(5) = 0.0;
  x_aug(6) = 0.0;
  ...
  double eq_const = sqrt(lambda + n_aug);

  for (int i = 0; i < n_aug; i++)
  {
    Xsig_aug.col(i+1) = x_aug + (eq_const * A.col(i));
    Xsig_aug.col(i+n_aug+1) = x_aug - (eq_const * A.col(i));
  }
  /**
   * Student part end
   */
  ...
  // print result
  std::cout << "Xsig_aug rows: " << Xsig_aug.rows() << "\nXsig_aug cols: " << Xsig_aug.cols()
...
void UKF::SigmaPointPrediction(MatrixXd* Xsig_out) {
  ...
  for (size_t i = 0; i < Xsig_pred.cols(); i++)
  {
    ...
    double yaw_k_dot = Xsig_aug(4,i);

    double acc_noise = Xsig_aug(5,i);
    double yaw_dot_dot_noise = Xsig_aug(6,i);
    ...
    Xsig_pred.col(i) = Xsig_aug.col(i).head(n_x) + f + nu;
  }
